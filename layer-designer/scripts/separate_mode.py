#!/usr/bin/env python3
"""
Separate Mode - Automated workflow for generating layers from a reference image.

This script automates the workflow when the user provides a reference image
instead of going through the standard 8-phase generation pipeline:

1. Create size_plan.json from reference image dimensions
2. Generate all layers in PL (precise_layout) mode from the reference image
3. Run transparency check + rembg matting + auto-crop on all layers
4. Run detect_layer_positions.py to match layers to their positions in the reference
5. Generate enhanced_layer_plan.json with detected layouts applied
...
Usage:
    python scripts/separate_mode.py \
        ...
        [--layer-plan path/to/layer_plan.json] \
        [--quality low] \
        [--parallel]

Requirements:
- layer_plan.json must already exist (generated by agent visual analysis)
- All non-background layers should have precise_layout: true
- Reference image is used as the source for all PL mode generations
"""
